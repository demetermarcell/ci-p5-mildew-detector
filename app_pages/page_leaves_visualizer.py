...
def page_leaves_visualizer_body():
    ...
    st.info(
        f"* The client requires a visual investigation that demonstrates how healthy and infected cherry leaves differ.")
    
    version = 'v1'
    if st.checkbox("Difference between average and variability image"):
      
      avg_infected = plt.imread('outputs/v1/avg_var_powdery_mildew.png')
      avg_healthy = plt.imread('outputs/v1/avg_var_healthy.png')

      st.warning(
        f"* The results confirm a clear visual distinction between infected and healthy leaves. Infected samples exhibit more surface irregularities and discoloration.")
    ...
    if st.checkbox("Image Montage"): 
      ...
      labels = os.listdir(my_data_dir+ '/valiation')
